...
#include "../include/hansen_hw_regs.h" // Shared Contract
...
static int hansen_open(struct inode *inode, struct file *file) { return 0; }

static int hansen_release(struct inode *inode, struct file *file) { return 0; }

// Write to device (User Space -> Kernel -> MMIO)
static ssize_t hansen_write(struct file *file, const char __user *buf,
...
// Read from device
// Read from device
static ssize_t hansen_read(struct file *file, char __user *buf, size_t count,
                           loff_t *ppos) {
  u32 val;
  loff_t offset = *ppos;
...
static int hansen_probe(struct pci_dev *pdev, const struct pci_device_id *id) {
  ...
  if (IS_ERR(my_hansen_dev->class))
    goto err_cdev_del;

  device_create(my_hansen_dev->class, NULL, my_hansen_dev->dev_major, NULL,
                "hansen0");
